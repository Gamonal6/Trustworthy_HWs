...
int main(int argc, char **argv)
{
    ...
    if (!read_bn_hex(argv[1], &x_b)) {
        fprintf(stderr, "Error: read_bn_hex(x_b) failed\n");
        goto cleanup;
    }
    ...
    if (!read_bn_hex(argv[3], &p_b)) {
        fprintf(stderr, "Error: read_bn_hex(p_b) failed\n");
        goto cleanup;
    }
    ...
    if (!E_b) {
        fprintf(stderr, "Error: EC_POINT_new(E_b) failed\n");
        goto cleanup;
    }
    if (!EC_POINT_mul(group, E_b, NULL, P, p_b, ctx)) {
        fprintf(stderr, "Error: EC_POINT_mul(E_b) failed\n");
        goto cleanup;
    }
    // write Bob's ephemeral public point E_b to file
    if (!write_point_hex("bob_ephemeral_Eb.txt", group, E_b)) {
        fprintf(stderr, "Error: write_point_hex(E_b) failed\n");
        goto cleanup;
    }
    ...
    if (!EC_POINT_mul(group, temp1, NULL, U_a, h_A, ctx)) {
        fprintf(stderr, "Error: EC_POINT_mul(h_A*U_a) failed\n");
        goto cleanup;
    }
    ...
    if (!EC_POINT_mul(group, temp2, NULL, temp1, x_b, ctx)) {
        fprintf(stderr, "Error: EC_POINT_mul(x_b*temp1) failed\n");
        goto cleanup;
    }
    // compute temp1 = p_b * E_a  (ephemeral DH component)
    if (!EC_POINT_mul(group, temp1, NULL, E_a, p_b, ctx)) {
        fprintf(stderr, "Error: EC_POINT_mul(p_b*E_a) failed\n");
        goto cleanup;
    }
